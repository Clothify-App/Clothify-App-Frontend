...
const Cropper = ({setimgURL}) => {
	...
	const [crop, setCrop] = useState({ unit: 'px',aspect: 5/7,
		width: 300,
		height: 480 });
	...
	return (
		...
		<center className='flex relative min-h-[60vh]'>
			...
			style={{display:"none"}}
			/>
			<button onClick={() => document.getElementById('imageIP').click()} className="border-2 border-black px-4 py-2 text-white text-bold rounded-md bg-black">Select Image</button>
			<br />
			<br />
			<div>
			{src && (
				<div>
				<ReactCrop src={src} aspect={5/7}  maxHeight={480} maxWidth={300} 
					crop={crop} onChange={crop => setCrop(crop)}>
			<img src={src} alt="productImage" onLoad={handleImageLoad} className=''/>
			</ReactCrop>
				<br />
				<button onClick={cropImageNow} className='p-2 bg-blue-900 text-white px-3 rounded'>Crop</button>
				<br />
				<br />
				</div>
			)}
			...
			<div className="flex-[0.5] flex flex-col justify-between items-center">
				<h3 className="mb-7 text-center font-bold opacity-70">Cropped Image: </h3>
				...
				<div className="uploadSection flex-1 flex justify-center items-center">
					<div className="container max-w-[200px] flex m-auto">
						<button className="p-2 border-black border-2 rounded-md mt-5 bg-black text-white" onClick={handleImageUploading}>upload Image</button>
					</div>
				</div>
			</div>
		</center>
