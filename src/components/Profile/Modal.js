  ...
  return (
    ...
    <div className="h-full flex items-center justify-center relative">
      ...
      <div className="cropped_Image flex-[0.5] flex justify-center items-center">
          {
              imgURL && (<img src={imgURL} className="object-contain h-[300px] w-[300px]"/>)
          }
      </div>
      </div>
      <div
    ...
    <div className="btns flex justify-between items-center">
        <button className="border-2 border-black px-5 rounded-md py-1 hover:shadow-lg" onClick={()=>{setToggle(1)}}>Previous</button>
        <button className="border-2 border-black px-5 rounded-md py-1 hover:shadow-lg" onClick={() => setToggle(3)}>Continue</button>
    </div>
</div>
</>
  );
